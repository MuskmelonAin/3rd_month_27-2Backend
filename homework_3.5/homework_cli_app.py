import argparse
from homework_db_helper import DBHelper
...
def main():
    ...
    if args.cmd == 'add':
        ...
        print("---------------------------------------------------")
        for r in rows:
            print(f"{r['id']:2d} | {r['name']:<10} | {r['age'] or '-':>3} | {r['email'] or '-'} | {r['city'] or '-'}")

    elif args.cmd == 'find':
        rows = db.find_users(args.keyword)
        if rows:
            for r in rows:
                print(f"{r['id']}: {r['name']} ({r['age']}), {r['email']} , {r['city']}")
        else:
...
# cd homework_3.5
# python homework_cli_app.py add --name "Anna" --age 30 --email anna@example.com --city "Moscow"
# python homework_cli_app.py update --id 1 --city "Санкт-Петербург"
# python homework_cli_app.py list
