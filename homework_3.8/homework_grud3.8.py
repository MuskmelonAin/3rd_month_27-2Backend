...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QInputDialog, QDateEdit, QComboBox
)
from PyQt6.QtCore import QDate

import homework_8_crud
...
class UserApp(QWidget):
    def __init__(self):
        ...
        self.status_input.addItems(["Активен", "Неактивен", "Заблокирован"])
        ...
        self.layout.addWidget(QLabel("Статус (Активен, Неактивен, Заблокирован):"))
    ...
    def add_user(self):
        ...
        try:
            conn = homework_8_crud.connect_db()
            conn.execute("INSERT INTO users (name, age, email, phone, birth_date, status) VALUES (?, ?, ?, ?, ?, ?)",
                         (name, age, email or None, phone or None, birth_date, status))
            conn.commit()
            conn.close()
    ...
    def update_user(self):
        ...
        if ok:
            ...
            count = homework_8_crud.update_user(user_id, name, age, email, phone, birth_date, status)
            if count:
    ...
    def show_users(self):
        ...
        self.output.clear()
        for u in users:
            self.output.append(f"{u[0]} | {u[1]} | {u[2]} | {u[3] or '-'} | {u[4] or '-'} | {u[5] or '-'} | {u[6]}")
        conn.close()

    def find_user(self):
        keyword, ok = QInputDialog.getText(self, "Поиск", "Введите имя/email/телефон/дату рождения/статус:")
        if ok and keyword:
            conn = homework_8_crud.connect_db()
            sql = """
                SELECT * FROM users
                WHERE name LIKE ? OR email LIKE ? OR phone LIKE ? OR birth_date LIKE ? OR status LIKE ?
            """
            results = conn.execute(sql, [f"%{keyword}%"] * 5).fetchall()
            conn.close()
            self.output.clear()
            if results:
                for u in results:
