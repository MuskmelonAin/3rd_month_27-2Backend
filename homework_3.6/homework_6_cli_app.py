import argparse
from homework_6_db_helper import DBHelper
...
def parse_args():
    ...
    p.add_argument('--job',            help='Работа')
    ...
    p.add_argument('--job',           help='Новая работа')
...
def main():
    ...
    if args.cmd == 'add':
        ...
        print("---------------------------------------------------")
        for r in rows:
            print(f"{r['id']:2d} | {r['name']:<10} | {r['age'] or '-':>3} | {r['email'] or '-'} | {r['job'] or '-'}")

    elif args.cmd == 'find':
        rows = db.find_users(args.keyword)
        if rows:
            for r in rows:
                print(f"{r['id']}: {r['name']} ({r['age']}), {r['email']} , {r['job']}")
        else:
...
# cd homework_3.6
# python homework_6_cli_app.py add --name "Олег" --age 35 --email oleg@example.com --job "Разработчик"
# python homework_6_cli_app.py update --id 1 --job "Тимлид"
# python homework_6_cli_app.py list
